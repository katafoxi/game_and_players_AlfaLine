...
class PlayerItem(BaseModel):
    ...
    date_change: datetime = None


class GameItem(BaseModel):
    name: str = Field(max_length=254)
    players: List[PlayerItem] = None
    date_create: datetime = None
    date_change: datetime = None
...
def create_new_player(player: PlayerItem,
    ...
    for symbol in new_player.name:
        if symbol not in 'abcdef123456789':
            return HTTPException(status_code=400, detail="Неподходящее имя. Используйте буквы a-f и цифры 0-9")

    try:
        validate_email(new_player.email)
    except ValidationError as e:
        return HTTPException(status_code=400, detail="некорректный Email-адрес")

    if (Player.objects.filter(name=new_player.name).count() == 1 or
            Player.objects.filter(email=new_player.email).count() == 1):
        return HTTPException(status_code=400, detail="player with such name or email already exists")
    new_player.save()
...
def create_new_game(game: GameItem, Authorize: AuthJWT = Depends()):
    ...
    if Game.objects.filter(name=new_game.name).count() == 1:
        return HTTPException(status_code=400, detail="game with such name already exists")
    new_game.save()
...
def add_player_to_game(game_id: int, player_id: int, Authorize: AuthJWT = Depends()):
    ...
    """
    Authorize.jwt_required()
    if Player.objects.filter(pk=player_id).count() != 1:
        return HTTPException(status_code=400, detail=f"Player with id={player_id} does not exist")
    if Game.objects.filter(pk=game_id).count() != 1:
        return HTTPException(status_code=400, detail=f"Game with id={game_id} does not exist")

    game = Game.objects.get(pk=game_id)
    if game.players.count() >= 5:
        return HTTPException(status_code=400, detail=f"There are no empty seats in the game with id={game_id}")

    player = Player.objects.get(pk=player_id)
    if player_id in [player.id for player in game.players.all()]:
        return HTTPException(status_code=400, detail=f"Player is already in the game with id={game_id}")
